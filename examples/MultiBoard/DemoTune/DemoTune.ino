...
byte resetPins[BOARD_COUNT] = { 8, 5 };
byte addressPins[BOARD_COUNT] = { 9, 6 };
byte latchPins[BOARD_COUNT] = { 10, 7 };
...
void setup() {
  ...
  // Setup channels 0, 1 and 2.
  Instrument piano2 = opl2.loadInstrument(INSTRUMENT_PIANO2);
  for (int i = 0; i < 3; i ++) {
    opl2.setInstrument(i, piano2);
    opl2.setBlock(i, 4);
  }
}
...
void loop() {
  for (int i = 0; i < 3; i ++) {
    int ms = millis();
    opl2.selectBoard(0);
    if (ms >= music[i].releaseTime && opl2.getKeyOn(music[i].channel)) {
    ...
    if (ms >= music[i].nextNoteTime && music[i].data->charAt(music[i].index) != 0) {
      parseTune(&music[i]);
    }
  }
  delay(1);
